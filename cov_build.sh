...
#Add code support for memleak
cd $WORKDIR
git clone https://github.com/JagadheesanD/memleakutil.git
cd memleakutil/
ls -l
make -f Makefile.raw
cp libmemfnswrap.so /usr/local/lib
cp ./inc/memfns_wrap.h /usr/local/include
cd ..
ls -l
...
AM_LDFLAGS="-L/usr/local/lib -lrbus -lsecure_wrapper -lcurl -lrfcapi -lrdkloggers -llibparodus  -lnanomsg  -lIARMBus -lWPEFrameworkPowerController -lds -ldshalcli -ldshalsrv  -lwrp-c -lwdmp-c -lprocps -ltrower-base64 -lcimplog -lsoup-3.0 -L/usr/lib/x86_64-linux-gnu -lyajl -L/usr/local/lib/x86_64-linux-gnu -ltinyxml2 -lmemfnswrap"  CXXFLAGS="-fpermissive -DPARODUS_ENABLE -DUSE_REMOTE_DEBUGGER"
make install
