...
if [ "$TEST_MODE" = "thunder" ]; then
    ...
    THUNDER_CXXFLAG="-DUSE_THUNDER_CLIENT"
    MODE_LDFLAGS=""
else
    CONFIGURE_THUNDER_OPT="--enable-thunder=no"
    THUNDER_CXXFLAG=""
    MODE_LDFLAGS="-lds -ldshalcli -ldshalsrv"
fi
...
autoreconf -i
./configure  --enable-IPv6=yes --enable-wifi=yes $CONFIGURE_THUNDER_OPT
...
AM_LDFLAGS="-L/usr/local/lib -lrbus -lsecure_wrapper -lcurl -lrfcapi -lrdkloggers -llibparodus -lglib-2.0 -lnanomsg  -lIARMBus -lWPEFrameworkPowerController -lwrp-c -lwdmp-c -lprocps -ltrower-base64 -lcimplog -lsoup-3.0 -L/usr/lib/x86_64-linux-gnu -lyajl -L/usr/local/lib/x86_64-linux-gnu -ltinyxml2 $MODE_LDFLAGS"  CXXFLAGS="-fpermissive -DPARODUS_ENABLE -DUSE_REMOTE_DEBUGGER $THUNDER_CXXFLAG" \
    install
