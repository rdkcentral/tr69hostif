...
int hostIf_IPInterface::set_Interface_Enable (int value)
{
    ...
    {
        snprintf (cmd,BUFF_LENGTH, "ifconfig %s up", nameOfInterface);
...
int hostIf_IPInterface::set_Interface_Mtu (unsigned int value)
{
    ...
    char cmd[BUFF_LENGTH] = { 0 };
    snprintf (cmd,BUFF_LENGTH, "ifconfig %s mtu %d", nameOfInterface, value);
    if (system (cmd) < 0)
        return NOK;
...
void hostIf_IPInterface::closeAllInstances()
{
    ...
    if(ifHash)
    {
        ...
        while(current)
        {
            hostIf_IPInterface* pDev = (hostIf_IPInterface *)current->data;
...
int hostIf_IPInterface::get_Interface_Type(HOSTIF_MsgData_t *stMsgData, bool *pChanged)
{
    ...
    strncpy(backupType,stIPInterfaceInstance.type,_BUF_LEN_16-1 );
    stMsgData->paramtype = hostIf_StringType;
