...
void hostIf_IP::closeAllInstances()
{
    if(ifHash)
    {
        ...
        while(current)
        {
            hostIf_IP* pDev = (hostIf_IP *)current->data;
...
{
    g_mutex_init(&hostIf_IP::m_mutex);
    g_mutex_lock(&hostIf_IP::m_mutex);
}
...
char* hostIf_IP::getVirtualInterfaceName (struct if_nameindex *phy_if_list, unsigned int virtual_if_index, char* virtual_if_name)
{
    ...
    unsigned int virtual_if_count = 0;
    char *p, *v;
    for (struct ifaddrs *ifa_node = ifa; ifa_node; ifa_node = ifa_node->ifa_next)
    {
        if (ifa_node->ifa_addr->sa_family == AF_INET) // virtual interfaces are IPv4-specific, so use IPv4 address family to hunt for them.
...
unsigned int hostIf_IP::getNumOfActivePorts(void) 
{
   ...
   std::ifstream tcpFile("/proc/net/tcp");
    if (!tcpFile.is_open()) {
        RDK_LOG(RDK_LOG_ERROR,LOG_TR69HOSTIF,"%s(): /proc/net/tcp File Open Error\n", __FUNCTION__);
        return 0;
    }
    ...
    while (std::getline(tcpFile, line)) 
    {
        ...
        // Check if the fourth field is "0A" or "01"
        if (field == "0A" || field == "01") {
            ++noOfActivePorts;
        }
    }
...
int hostIf_IP::get_Device_IP_Fields(EIPMembers ipMem)
{
    ...
    case eIpIPv4Enable:
    case eIpIPv4Status:
        fp = v_secure_popen("r", "ifconfig | egrep 'inet addr:' | wc -l");
...
int hostIf_IP::set_Device_IP_IPv4Enable(HOSTIF_MsgData_t *stMsgData)
{
    ...
    {
        rc = v_secure_system("background ifup -a");
	if(rc != -1)
	{
    ...
    {
        rc = v_secure_system("background ifdown -a");
	if(rc != -1)
        {
