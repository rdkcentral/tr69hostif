...
GMutex hostIf_IPv4Address::m_mutex;
...
int hostIf_IPv4Address::setIpOrMask(int interfaceNo, char *value, const char* ipOrMask)
{
    ...
    if (0 != strcmp (value, ""))
    {
        ...
        {
            v_secure_system("background ifconfig %s %s %s", nameOfInterface, ipOrMask, value);
...
void hostIf_IPv4Address::closeAllInstances()
{
    ...
    if(ifHash)
    {
        ...
        while(current)
        {
            hostIf_IPv4Address* pDev = (hostIf_IPv4Address *)current->data;
...
{
    g_mutex_init(&hostIf_IPv4Address::m_mutex);
    g_mutex_lock(&hostIf_IPv4Address::m_mutex);
}

void hostIf_IPv4Address::releaseLock()
{
    RDK_LOG(RDK_LOG_INFO,LOG_TR69HOSTIF,"[%s:%d] Unlocking mutex...  \n", __FUNCTION__, __LINE__);
    g_mutex_unlock(&hostIf_IPv4Address::m_mutex);
}
...
*/
    ...
    strncpy (backupAddressingType, addressingType, BUFF_LENGTH_16-1);
    backupAddressingType[BUFF_LENGTH_16-1] = '\0';
...
int hostIf_IPv4Address::set_IPv4Address_Enable(HOSTIF_MsgData_t *stMsgData, int subInstanceNo)
{
    ...
    {
        v_secure_system("background ifconfig %s 0.0.0.0", nameOfInterface);
        RDK_LOG(RDK_LOG_INFO,LOG_TR69HOSTIF,"[%s(),%d] IPv4Address Disabled   \n",__FUNCTION__,__LINE__);

    }
    if(TRUE == get_boolean(stMsgData->paramValue))
    {
        v_secure_system("background ifdown %s && ifup %s", nameOfInterface, nameOfInterface);
