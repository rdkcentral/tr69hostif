...
void hostIf_IPv6Address::closeAllInstances()
{
    ...
    if(ifHash)
    {
        ...
        while(current)
        {
            hostIf_IPv6Address* pDev = (hostIf_IPv6Address *)current->data;
...
*/
    ...
    snprintf ((char*) positionAfterInstanceNumber,strlen((char*) positionAfterInstanceNumber), "%s%d.", "IPv6Prefix.", subInstanceNo);
 ...
 */
bool hostIf_IPv6Address::get_ipv6address_lifetime (unsigned int instance, const char* search_string, long int& lifetime)
{
    LOG_ENTRY_EXIT;
    ...
    char *cmd = (char *)malloc(BUFF_LENGTH_64 * sizeof(char));
    char resultBuff[BUFF_LENGTH_128] = { 0 };
    asprintf(&cmd, "ip -6 -o addr show %s | awk '{i++;if(i==%d){print}}'", nameOfInterface, instance);

    FILE* fp = v_secure_popen("r","ip -6 -o addr show %s | awk '{i++;if(i==%d){print}}'", nameOfInterface, instance);
    if (fp == NULL)
    {
        RDK_LOG (RDK_LOG_ERROR, LOG_TR69HOSTIF, "[%s]: cannot run command [%s]\n", __FUNCTION__, cmd);
        return false;
    }
    if (fgets (resultBuff, BUFF_LENGTH_128, fp) == NULL)
    {
...
int hostIf_IPv6Address::get_IPv6Prefix_Prefix(HOSTIF_MsgData_t *stMsgData,int subInstanceNo, bool *pChanged)
{
    ...
    char *prefix = NULL;
    prefix = (char *)malloc(BUFF_LENGTH_64 * sizeof(char));
    asprintf(&prefix, "%s/%u", prefix_address, prefix_length);
