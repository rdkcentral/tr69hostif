...
hostIf_STBServiceHDMI* hostIf_STBServiceHDMI::getInstance(int dev_id)
{
    if (!ifHash) {
        ...
        if (!ifHash) {
            RDK_LOG(RDK_LOG_WARN, LOG_TR69HOSTIF,
                    "[%s:%s:%d]: HDMI hash not initialized (Thunder may be inactive)\n",
    ...
    if (!pRet)
    {
        RDK_LOG(RDK_LOG_INFO, LOG_TR69HOSTIF,
                "[%s:%s:%d]: Retry HDMI hash build for dev_id=%d\n",
                __FILE__, __FUNCTION__, __LINE__, dev_id);
        buildPortNameHash();
        if (ifHash)
        {
            pRet = (hostIf_STBServiceHDMI*)g_hash_table_lookup(ifHash, (gpointer)(intptr_t)dev_id);
        }
    }
...
{
    RDK_LOG(RDK_LOG_INFO, LOG_TR69HOSTIF, "[%s] Getting HDMI param: %s\n", __FUNCTION__, paramName);
