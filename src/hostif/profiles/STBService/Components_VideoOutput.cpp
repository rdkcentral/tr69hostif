...
hostIf_STBServiceVideoOutput* hostIf_STBServiceVideoOutput::getInstance(int dev_id)
{
    ...
    if(!pRet)
    {
        try {
            ...
            RDK_LOG(RDK_LOG_INFO, LOG_TR69HOSTIF, "strVideoPort: %s\n", strVideoPort.c_str());
...
{
    try {
        /*By default some cpc device  only supports HDMI */
...
{
    try {
        snprintf(stMsgData->paramValue,TR69HOSTIFMGR_MAX_PARAM_LEN,"%s",vPort.getResolution().getName().c_str());
        RDK_LOG(RDK_LOG_DEBUG,LOG_TR69HOSTIF,"[%s] : Value: %s \n",__FUNCTION__, stMsgData->paramValue);
        stMsgData->paramtype = hostIf_StringType;
        stMsgData->paramLen = strlen(stMsgData->paramValue);
...
{
    try {
        snprintf(stMsgData->paramValue,TR69HOSTIFMGR_MAX_PARAM_LEN,"%s",vPort.getName().c_str());
        RDK_LOG(RDK_LOG_DEBUG,LOG_TR69HOSTIF,"[%s] : Value: %s \n",__FUNCTION__, stMsgData->paramValue);
        stMsgData->paramtype = hostIf_StringType;
        stMsgData->paramLen = strlen(stMsgData->paramValue);
