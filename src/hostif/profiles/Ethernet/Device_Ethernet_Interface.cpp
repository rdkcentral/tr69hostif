...
static int get_Device_Ethernet_Interface_Fields(unsigned int ethInterfaceNum,EEthInterfaceMembers ethInterfaceMem)
{
    ...
    case eEnable:
        ...
        snprintf(cmd,BUFF_LENGTH,"cat /sys/class/net/%s/carrier", ethernetInterfaceName);
    ...
    case eUpstream:
        ...
        if(fgets(resultBuff,1024,fp)!=NULL)
        {
            sscanf(resultBuff,"%1d",&hostIf_EthernetInterface::stEthInterface.upStream );
        }
#ifdef YOCTO_BUILD
        v_secure_pclose(fp);
#else
    ...
    case eMACAddress:
        ...
        snprintf(cmd,BUFF_LENGTH,"cat /sys/class/net/%s/address", ethernetInterfaceName);
#ifdef YOCTO_BUILD
        ...
        if(fgets(resultBuff,1024,fp)!=NULL)
        {
            sscanf(resultBuff,"%23s",hostIf_EthernetInterface::stEthInterface.mACAddress);
        }
#ifdef YOCTO_BUILD
        v_secure_pclose(fp);
#else
    ...
    case eMaxBitRate:
        ...
        snprintf(cmd,BUFF_LENGTH,"cat /sys/class/net/%s/speed", ethernetInterfaceName);
#ifdef YOCTO_BUILD
        ...
        if(fgets(resultBuff,1024,fp)!=NULL)
        {
            sscanf(resultBuff,"%10d",&hostIf_EthernetInterface::stEthInterface.maxBitRate);
        }
#ifdef YOCTO_BUILD
        v_secure_pclose(fp);
#else
    ...
    case eDuplexMode:
        ...
        snprintf(cmd,BUFF_LENGTH,"cat /sys/class/net/%s/duplex", ethernetInterfaceName);
#ifdef YOCTO_BUILD
        ...
        if(fgets(resultBuff,1024,fp)!=NULL)
        {
            sscanf(resultBuff,"%15s",hostIf_EthernetInterface::stEthInterface.duplexMode);
        }
#ifdef YOCTO_BUILD
        v_secure_pclose(fp);
#else
