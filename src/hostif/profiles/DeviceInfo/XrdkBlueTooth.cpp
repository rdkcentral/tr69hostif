...
#define BTRMGR_QUERY_INTERVAL 10
...
int hostIf_DeviceInfoRdk_xBT::handleSetMsg(HOSTIF_MsgData_t *stMsgData)
{
    ...
    try {
        ...
        if(NULL == path) {
            RDK_LOG(RDK_LOG_ERROR,LOG_TR69HOSTIF,"[%s:%d]xBlueTooth: Failed : Parameter is NULL \n",  __FUNCTION__, __LINE__);
...
int hostIf_DeviceInfoRdk_xBT::getPairedDevice_Profile(HOSTIF_MsgData_t *stMsgData) {
    ...
    try {
        ...
        if((devIndex && pairedDevList.m_numOfDevices) && (devIndex <= pairedDevList.m_numOfDevices)) {
            ...
            {
                strncat(serviceInfo, pairedDevList.m_deviceProperty[devIndex-1].m_serviceInfo.m_profileInfo[i].m_profile, TR69HOSTIFMGR_MAX_PARAM_LEN-1);
                if ( (i + 1) < pairedDevList.m_deviceProperty[devIndex-1].m_serviceInfo.m_numOfService)
                    strncat(serviceInfo, ",", 2);
            }
...
int hostIf_DeviceInfoRdk_xBT::getDeviceInfo_Profile(HOSTIF_MsgData_t *stMsgData)
{
    ...
    try {
        ...
        if(handle_devInfo && numOfService) {
            ...
            {
                strncat(serviceInfo, deviceProperty.m_serviceInfo.m_profileInfo[i].m_profile, TR69HOSTIFMGR_MAX_PARAM_LEN-1);
...
void hostIf_DeviceInfoRdk_xBT::checkForUpdates()
{
    ...
    if (retCode != IARM_RESULT_SUCCESS || isRegistered == 0 ) {
        RDK_LOG(RDK_LOG_ERROR, LOG_TR69HOSTIF, "[%s]xBlueTooth: BTRMgrBus not registered with IARM ret_code = %d isRegistered = %d\n", __FUNCTION__,retCode,isRegistered);
    return;
    }
