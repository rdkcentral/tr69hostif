...
GMutex hostIf_DeviceProcess::m_mutex;

GMutex hostIf_DeviceProcess::m_libproc_lock;
...
#endif
  g_mutex_init(&hostIf_DeviceProcess::m_libproc_lock);
...
{
    g_mutex_clear(&hostIf_DeviceProcess::m_libproc_lock);
}
...
void hostIf_DeviceProcess::closeAllInstances()
{
    if(ifHash)
    {
        ...
        while(current)
        {
            hostIf_DeviceProcess* pDev = (hostIf_DeviceProcess *)current->data;
...
{
    g_mutex_init(&hostIf_DeviceProcess::m_mutex);
    g_mutex_lock(&hostIf_DeviceProcess::m_mutex);
}
...
/**
 &hostIf_DeviceProcess::* @brief This function provides The current state of the process.
 *
 * @param[out] stMsgData TR-069 Host interface message request.
 * @param[in] pChanged  Status of the operation.
 *
