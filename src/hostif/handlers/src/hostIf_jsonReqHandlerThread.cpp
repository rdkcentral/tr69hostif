...
#endif
    if(parser == NULL){
         RDK_LOG(RDK_LOG_ERROR,LOG_TR69HOSTIF,"[%s:%s] Failed to get parse n", __FUNCTION__, __FILE__);
         return context.list;
    }
...
void hostIf_HttpServerStop()
{
    ...
    if(server) {
        ...
        RDK_LOG(RDK_LOG_INFO,LOG_TR69HOSTIF,"SERVER: No server instance to stop.\n");
    }
