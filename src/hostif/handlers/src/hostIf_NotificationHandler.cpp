...
void NotificationHandler::pushNotification(const char* destination, const char* payload)
{
    ...
    {
        paramNotify->notifyDest = strdup (destination);
...
void NotificationHandler::push_device_deepsleep_notifications(const char* deepsleep_status, const char* event_time)
{
    cJSON *notifyPayload = NULL;
    ...
    if(notifyPayload != NULL)
    {
        ...
        char *fw_name = NULL;
        HOSTIF_MsgData_t stMgsData = {0};
        hostIf_DeviceInfo::getInstance(0)->get_X_RDK_FirmwareName(&stMgsData);
        fw_name = stMgsData.paramValue;
        RDK_LOG (RDK_LOG_DEBUG, LOG_PARODUS_IF, "fw_name is %s\n", fw_name);

        if(fw_name != NULL)
        {
            cJSON_AddStringToObject(notifyPayload,"fw-name", fw_name);
        }

        if(deepsleep_status !=NULL)
        {
            cJSON_AddStringToObject(notifyPayload,"status", deepsleep_status);
        }

        if(event_time !=NULL)
        {
            cJSON_AddStringToObject(notifyPayload,"event-time", event_time);
        }
