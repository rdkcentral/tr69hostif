...
int XRdkCentralT2::handleSetMsg(HOSTIF_MsgData_t *stMsgData)
{
    ...
    if(strcasecmp(stMsgData->paramName, "Device.X_RDKCENTRAL-COM_T2.ReportProfiles") == 0) {
        if(strlen(stMsgData->paramValue) != 0 ) {
            if (stMsgData->isLengthyParam && NULL != stMsgData->paramValueLong) {
                ...
                setRbusStringParam((const char*)T2_REPORT_PROFILE_PARAM, stMsgData->paramValueLong);

                /*Cross check*/
                if(OK == getRbusStringParam((const char*)T2_REPORT_PROFILE_PARAM, &paramValueGetVal)) {
                    //Check length
                ...
                setRbusStringParam((const char*)T2_REPORT_PROFILE_PARAM, stMsgData->paramValue);
            }
        }
        ret = OK ;
...
int XRdkCentralT2::handleGetMsg(HOSTIF_MsgData_t *stMsgData)
{
    ...
    if(strcasecmp(stMsgData->paramName, "Device.X_RDKCENTRAL-COM_T2.ReportProfiles") == 0)
    {
        ...
        if(OK == getRbusStringParam((const char*)T2_REPORT_PROFILE_PARAM, &paramValue)) {
