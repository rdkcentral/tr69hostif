...
bool IniFile::flush()
{
    // FIXME: truncating everytime is bad for flash ingeneral
    ...
    for (map<string,string>::const_iterator it = m_dict.begin(); it != m_dict.end(); ++it) {
        outputStream << it->first << '=' << it->second << endl;
    }

    // even though desctructor closes it, for clarity purpose good to close
    outputStream.close();
