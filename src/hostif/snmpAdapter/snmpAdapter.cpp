...
int hostIf_snmpAdapter::GetStdoutFromSnmpgetCommand(const char *community, const char *address, const char *oid, string &consoleString)
{
    FILE * stream;
    ...
    consoleString.clear();
    stream = v_secure_popen("r", "snmpget -OQ -Ir -v 2c -c %s %s %s", community, address, oid);
    if (stream == NULL) {
        return -1;
    }
    else
...
int hostIf_snmpAdapter::get_ValueFromSNMPAdapter(HOSTIF_MsgData_t *stMsgData)
{
    ...
    if(stMsgData)
    {
	    ...
	    ret = GetStdoutFromSnmpgetCommand( SNMP_COMMUNITY, (value.compare(interface_STB) == 0) ? SNMP_AGENT_STB_IP_ADDRESS:SNMP_AGENT_CM_IP_ADDRESS , it->second[0].first.c_str(), consoleString);
            if (ret == OK)
            {
        ...
        if (it != tr181Map.end())
        {
            ...
            switch(stMsgData->paramtype)
            {
                ...
                case hostIf_IntegerType:
                    if (value.compare(interface_STB) == 0)
                    {
                        CMD(cmd, BUFF_LENGTH_256, "snmpset -v 2c -c %s %s %s i %s",
                        SNMP_COMMUNITY, SNMP_AGENT_STB_IP_ADDRESS,
                        it->second[0].first.c_str(),
                        stMsgData->paramValue);
                    }
                ...
                case hostIf_UnsignedIntType:
                    if (value.compare(interface_STB) == 0)
                    {
                        CMD(cmd, BUFF_LENGTH_256, "snmpset -v 2c -c %s %s %s u %s",
                        SNMP_COMMUNITY,
