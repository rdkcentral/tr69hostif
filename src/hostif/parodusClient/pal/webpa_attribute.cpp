...
static WAL_STATUS get_AttribValues_tr69hostIf(HOSTIF_MsgData_t *ptrParam)
{
    ...
    retStatus = hostIf_GetAttributesMsgHandler(ptrParam);
    if(retStatus != 0) {
...
{
    errno_t safec_rc = -1;
    unsigned int sizeAttrArr = 1; // Currently support only Notification parameter
    unsigned int i = 0;
...
static WAL_STATUS setParamAttributes(const char *pParameterName, const AttrVal *attArr)
{
    ...
    for(i = 0; i < g_notifyListSize; i++)
    {
        ...
        {
            found = 1;
            RDK_LOG(RDK_LOG_DEBUG,LOG_PARODUS_IF,"[%s:%s:%d] Inside setParamAttributes, Param Found in Glist: %s \n", __FILE__, __FUNCTION__, __LINE__,pParameterName);
            break;
        }
    }
    if(!found)
