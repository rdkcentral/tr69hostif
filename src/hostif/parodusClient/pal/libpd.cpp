...
static void get_parodus_url(char *parodus_url, char *client_url)
{
    ...
    if (fp != NULL)
    {
        ...
        else
        {
            ...
            if(webpaCfgFile)
            {
                int readRet = fread(webpaCfgFile, 1, ch_count,fp);
                if(readRet < 0){
                    RDK_LOG(RDK_LOG_ERROR,LOG_PARODUS_IF,"[%s] Failed read from %s.\n", __FUNCTION__, webpaCfgFile);
                    fclose(fp);
                    free(webpaCfgFile);
                    return;
                }
    ...
    if(!getStatus)
    {
        strncpy(parodus_url, PARODUS_URL, strlen(parodus_url) + 1);
	parodus_url[strlen(parodus_url) +1] = '\0'; 
	strncpy(client_url, PAROUDUS_CLIENT_URL, strlen(client_url) + 1);
	client_url[strlen(client_url) + 1] = '\0';
    }
    ...
    while(1)
    {
        ...
        if(ret ==0)
        {
            ...
            if (fp != NULL) {
                ...
                RDK_LOG(RDK_LOG_INFO,LOG_PARODUS_IF,"File %s failed to open\n", webpa_start_tm_file);
            }
            break;
        }
        else
